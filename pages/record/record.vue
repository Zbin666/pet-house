...
<script setup>
	...
	const selected = ref({
		y: current.value.getFullYear(),
		m: current.value.getMonth(),
		d: current.value.getDate()
	})
	...
	const calendarDays = computed(() => {
		...
		for (let i = 0; i < 42; i++) {
			...
			const isToday = inMonth && (day === today.getDate()) && (month === today.getMonth()) && (year === today
				.getFullYear())
	...
	const weekDays = computed(() => {
		...
		const idx = days.findIndex(d => d.inMonth && d.day === selected.value.d && d.m === selected.value.m && d
			.y === selected.value.y)
	...
	function goCreate() {
		uni.navigateTo({
			url: '/pages/createRecord/createRecord'
		})
	}
	...
	function goRecordDetail(type) {
		...
		uni.navigateTo({
			url: `/pages/recordDetail/recordDetail?type=${type}&startDate=${startDate}&endDate=${endDate}`
		})
	}
	...
	const recordOptions = ref([{
			key: 'eating',
			title: '饮食',
			icon: '/static/record/eating.png'
		},
		{
			key: 'drinking',
			title: '饮水',
			icon: '/static/record/drinking.png',
			hasValue: true,
			value: 300,
			unit: 'ml'
		},
		...
		},
		{
			key: 'washing',
			title: '洗护',
			icon: '/static/record/washing.png'
		},
		{
			key: 'shit',
			title: '便便',
			icon: '/static/record/shit.png'
		},
		{
			key: 'noting',
			title: '记事',
			icon: '/static/record/noting.png'
		},
		{
			key: 'abnormal',
			title: '异常',
			icon: '/static/record/abnormal.png'
		},
		{
			key: 'medicine',
			title: '用药',
			icon: '/static/record/medicine.png'
		}
	])
	...
	async function fetchRemindersForSelectedDay() {
		try {
			...
			reminders.value = list.map(s => {
				...
				const title = s.content || (s.type === 'medicine' ? '用药提醒' : (s.type === 'vaccine' ? '疫苗提醒' : (
					s.type === 'wash' ? '洗护提醒' : '日常提醒')))
				// 映射颜色条样式类（与 legend 一致：d2=洗护, d3=疫苗）
				const typeClass = s.type === 'medicine' ? 't4' : (s.type === 'vaccine' ? 't3' : (s.type ===
					'wash' ? 't2' : 't1'))
				return {
					id: s.id,
					type: s.type,
