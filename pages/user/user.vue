		...
		<view class="profile-card">
			...
			<view class="profile-main" @tap="openSetting">
				<view class="avatar-container">
					<image class="avatar" :src="getUserAvatarSrc(userInfo?.avatarUrl)" mode="aspectFill"
						@error="onAvatarError" @load="onAvatarLoad"></image>
				...
				<view class="meta">
					...
					<view class="stats">
						<view class="stat"><text class="num">{{ stats.feeds || 0 }}</text><text class="label">动态</text>
						</view>
						<view class="divider"></view>
						<view class="stat"><text class="num">{{ stats.likes || 0 }}</text><text class="label">点赞</text>
						</view>
					</view>
				</view>
			...
			<view class="pet-grid">
				<view class="pet-card" v-for="p in pets" :key="p.id" @tap="goPetDetail(p)">
					...
					<view class="pet-information">
						<view class="pet-name">
							<text>{{ p.name }}</text>
							<image class="gender sm"
								:src="p.gender === 'male' ? '/static/user/male.png' : '/static/user/female.png'">
							</image>
						</view>
...
<script setup>
	...
	async function loadData() {
		try {
			...
			maxTogetherDays.value = pets.value.reduce((max, p) => {
				...
				if (!start) return max
				const days = Math.max(1, Math.floor((Date.now() - new Date(start).getTime()) / (1000 * 60 *
					60 * 24)))
	...
	function goPetDetail(p) {
		...
		uni.navigateTo({
			url: `/pages/petDetail/petDetail?pet=${payload}`
		})
	}
